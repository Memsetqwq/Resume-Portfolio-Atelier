...
> 给运维/用户自己看的:**怎么把履历裁缝专家正确部署到 Qclaw**,并确保 5 个 skill(4 个 style + huashu-design)都活着。
...
| `huashu-design` | `skills/huashu-design/SKILL.md` + `references/` | **跨场景辅助**(幻灯片/原型/动画/可视化);用户外溢需求时启用 | ✓ |
...
找到 workspace 里的 `openclaw.json`(通常在 `~/.openclaw/agents/<agent-id>/openclaw.json` 或 workspace 根),定位你的 agent 配置,在 `skills` 数组里**追加**这 5 项:
...
新会话第一句问 Agent:**"你的 5 个 skill(4 个 style + huashu-design)都在吗?做个自检"**。
...
✓ 5 个 skill(4 个 style + huashu-design)都在 Qclaw allowlist 里
...
### Q1:zip 上传后,5 个 skill(4 个 style + huashu-design)都不见了
...
- **本专家结构**:`AGENTS.md` §"技能自举"—— 主 Agent 启动时怎么自检 5 个 skill(4 个 style + huashu-design)
