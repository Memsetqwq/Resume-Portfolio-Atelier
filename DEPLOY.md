...
> 给运维/用户自己看的:**怎么把履历裁缝专家正确部署到 Qclaw**,并确保 4 个 style skill 都活着(huashu-design 作为内部参考资源,不进 allowlist)。
...
| `huashu-design` | `skills/huashu-design/SKILL.md` + `references/` | **内部参考资源**(v1.1.4 起融入每个 style,不再单列) | ✗(不需要) |
...
✓ huashu-design SKILL.md + references/ 在(内部参考资源,已融入各 style SKILL.md)
...
### Q6:`huashu-design/` 算 skill 吗?
A:v1.1.4 加强版起**不算**。它是**内部参考资源**,其设计原则被融入每个 style 的 SKILL.md 末尾"外溢需求"段;`skills/huashu-design/` 目录保留(SKILL.md + references/ 26 篇),主 Agent 直接读参考文件。Qclaw 不需要 allowlist,也不单列在 skills 数组里。
...
- **本专家结构**:`AGENTS.md` §"技能自举"—— 主 Agent 启动时怎么自检 4 个 style skill + 2 个内部参考(大厂风格 + huashu-design)
- **风格 skill 各自文档**:`skills/<风格名>/SKILL.md` × 4 —— 风格本身的视觉规则、触发词、避坑点、**外溢需求**(PPT/原型/动画/可视化)参考 huashu-design
