...
| [alchaincyf/huashu-design](https://github.com/alchaincyf/huashu-design) | MIT | `skills/huashu-design/` —— HTML 原生的设计 skill(原型/幻灯片/动画/可视化/20 设计哲学/5 维评审)。**只装 SKILL.md + LICENSE + README + references/**,完整版(含 scripts/ 和 assets/)见上游 |
...
├── skills/          ← 风格 skill 包(每个 SKILL.md 含"包装区间适配")
│   ├── 极简留白/
│   ├── 杂志编辑/
│   ├── 赛博未来/
│   ├── 手账拼贴/
│   ├── 大厂风格/   ← 用户不明确时的默认推荐
│   └── huashu-design/   ← 跨场景辅助 skill:幻灯片/原型/动画/可视化(MIT,见 INTEGRATION.md)
