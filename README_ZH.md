...
8. **零 emoji 默认(v1.1 强化)** —— 默认完全不用 emoji,改用内联 SVG 图标(走 `skills/flat-icons/`);你明确要求时单独审批,上限 3 个
...
| [tabler/tabler-icons](https://github.com/tabler/tabler-icons) | MIT | `skills/flat-icons/` —— 4000+ 平面图形图标库,内联 SVG 用于联系区/章节装饰/操作指示(替代几何字符 `◆` `▲` `■` `●`)。4 风格按各自视觉差异化使用,见 `skills/flat-icons/INTEGRATION.md` |
...
- [ ] 内容没有编造(对照用户原话)
- [ ] 配色对比度达标(色盲友好)
- [ ] **零 emoji**(默认),所有装饰用 `skills/flat-icons/` 的内联 SVG 平面图形;用户明确要求 emoji 时单独审批且不超过 3 个
...
│   └── flat-icons/   ← 内部参考资源(平面图形图标库,替代几何字符,主 Agent 内部读)
...
_Last updated: 2026-07-03, v1.1.4 加强:增加 flat-icons 平面图形素材库_